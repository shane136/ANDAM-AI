...
// Helper function to handle generateContent across candidate models with automatic retry on 503/429
async function generateContentWithFallback(ai: GoogleGenAI, baseParams: any) {
  ...
  for (let mIdx = 0; mIdx < CANDIDATE_MODELS.length; mIdx++) {
    ...
    // Attempt up to 2 tries per model if temporary 503/429 spike occurs
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        console.log(`[Gemini API] Calling model: ${modelName} (attempt ${attempt + 1})...`);
        ...
        const isTransient = errStatus === 503 || errStatus === 429 || errMsg.includes('503') || errMsg.includes('high demand') || errMsg.includes('overloaded');

        console.warn(`[Gemini API] Model ${modelName} failed (${errStatus}): ${errMsg.slice(0, 120)}`);

        if (isTransient && attempt === 0) {
          // Short jitter backoff before retrying once or moving to next model
          await new Promise((r) => setTimeout(r, 600));
          continue;
        }
        // If not transient or second attempt failed, immediately cascade to the next candidate model
        break;
      }
    }
  }
